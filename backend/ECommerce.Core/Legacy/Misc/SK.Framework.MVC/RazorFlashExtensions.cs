...
public static class RazorFlashExtensions
{
    private static string GetCssClassFor(FlashType type)
    {
        switch (type)
        {
            ...
            case FlashType.Notice: return "alert-warning";
    ...
    {
        object tmpData = self.TempData["FlashMessage_" + key];
        object tmpType = self.TempData["FlashMessageType_" + key];
    ...
    {
        object tmpData = self.TempData["FlashMessage_" + key];//ignore
        object flush = self.TempData["FlashMessageType_" + key]; //ignore
    ...
    {
        object tmpData = self.TempData["FlashMessage_" + key];//ignore
        object tmpType = self.TempData["FlashMessageType_" + key]; //ignore
    ...
    public static IHtmlContent FlashFormat(this IHtmlHelper self, string message, FlashType type)
    {
        string cssClass = GetCssClassFor(type);
        return new HtmlString($"<div class=\"alert {cssClass}\">{message}</div>");
    }
